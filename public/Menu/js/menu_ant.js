$(function() {
				//$('nav#menu').mmenu();
				menu =  $('nav#menu').mmenu(

				{	
					navbars		: [{
						position: "top",	
						height 	: 1,
						content : [ 
							
							'<img src="imagenes/wheel32.png" /><p class="navbartxt">bicimap.uy</p>',				
							
						]
					},
					{
			            position: "bottom",
	        		    content: [  	               			
	        		    	'<a href="mailto:bicimapuy@gmail.com" class="fa fa-envelope"></a>',	        		    	
	               			'<a class="fa fa-twitter"></a>',	               			
	               			'<a href="https://www.facebook.com/" target="_blank"class="fa fa-facebook"></a>'
	            		]
         			}
					],					
					extensions: ["multiline"],	
        			 onClick: {
             			close: false
        			 },
        			navbar:{
        				title: "Inicio"        				
        			},
        			offCanvas: {
               		  zposition : "next"
            		}
    	
			});

			API = $("#menu").data( "mmenu" ); // antes var API

			menu.on( 'click', 'a[class^="fa fa-twitter"]', function() {  // esto funciona
			 				//$('#twitter').show();
							$( "#twitter" ).toggle();
      						  //var API = $("#menu").data( "mmenu" );
       						  API.close();		
      						
			 				return false;
			 			}
			 );

			
        });
		
		$( window ).load(function() {

  			var $ruteo = $('input[name="ruteo"');
  			$ruteo.click(function() {  				
  				borrarPath();
	  			if ($ruteo.is( ':checked' ))
	  			{
	  				console.log("click ruteo si");
	  					routing = true;
	      				displayRoutingControl();        			      		      	
	      				API.close();	 			
	  			}
	  			else
	  			{	  				
	  				borrarRuta();
	  				removeRouting();	  				

	  			}
  				
  			});
  			//Pendientes
  			$(".pendientes").click(function(){
  				borrarPath();
    			routing = false;
      			loadPendientes();  
      			      		      		
      		API.close();	 
         
  			});
  			
  			
  			$( "#ptosi").click(function(){
  				console.log("Estoy en puntos de interes");
  				if (latlngmap != null)
  				{
  					latlngmap.lat = latlngmap.lat + 20;
  					latlngmap.lng = latlngmap.lng + 20;
	  				popupBIC = L.popup()
	            		.setLatLng([latlngmap.lat,latlngmap.lng])
	            		.setContent('<b><p>Bicicircuitos</b></p><p><p><font color=#3410E8>Bicisendas ▬▬▬▬</font></p>')
	            		.openOn(mapa);
            	}	
        
  			});

});